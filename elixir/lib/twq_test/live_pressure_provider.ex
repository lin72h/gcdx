defmodule TwqTest.LivePressureProvider do
  @moduledoc """
  Elixir-side helpers for the live M15 pressure-provider smoke artifact.

  This lane validates the guest-side live pressure view for `dispatch.pressure`
  and `dispatch.sustained`. It still does not claim a provider SPI or ABI. The
  contract remains pressure-only, with real generation and monotonic time in a
  probe-scoped artifact.
  """
  ...
  @capture_equal_fields ~w(label interval_ms duration_ms)
  ...
  def assert_ok!(%{failures: failures}) do
    raise "live pressure-provider comparison failed:\n" <> Enum.join(failures, "\n")
  end
  ...
  defp minimum_value(baseline, _ratio) when baseline <= 1, do: baseline * 1.0
  defp minimum_value(baseline, ratio), do: baseline * ratio
