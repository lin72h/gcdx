defmodule TwqTest.PressureProviderPreview do
  @moduledoc """
  Elixir-side helpers for the raw M15 pressure-provider preview smoke artifact.

  This lane validates the repo-local raw snapshot v1 preview surface. It still
  does not claim a provider SPI or ABI.
  """
  ...
  @capture_equal_fields ~w(label interval_ms duration_ms struct_version struct_size bucket_count)

  @required_true_fields ~w(
  ...
  def assert_ok!(%{failures: failures}) do
    raise "pressure-provider preview comparison failed:\n" <> Enum.join(failures, "\n")
  end
  ...
  defp minimum_value(baseline_value, _ratio)
       when is_integer(baseline_value) and baseline_value <= 1,
  ...
  defp normalize_nulls(value) when is_map(value) do
    value
    |> Enum.map(fn {key, entry} -> {key, normalize_nulls(entry)} end)
    |> Map.new()
  end
