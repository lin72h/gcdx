defmodule TwqTest.PressureProviderSession do
  @moduledoc """
  Elixir-side helpers for the M15 pressure-provider session smoke artifact.

  This lane validates a repo-local callable session surface that owns the base
  snapshot and generation sequencing while still returning the aggregate-only
  pressure view.
  """
  ...
  def assert_ok!(%{failures: failures}) do
    raise "pressure-provider session comparison failed:\n" <> Enum.join(failures, "\n")
  end
