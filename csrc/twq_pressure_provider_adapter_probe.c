...
emit_provider_error(const char *label, const char *stage, int error,
    uint64_t generation)
{
	printf("{\"kind\":\"pressure-provider-adapter-probe\",\"status\":\"error\","
	    "\"data\":{\"label\":\"%s\",\"stage\":\"%s\",\"rc\":%d,"
	    "\"generation\":%" PRIu64 "},\"meta\":{\"component\":\"c\","
	    "\"binary\":\"twq-pressure-provider-adapter-probe\"}}\n",
...
{
	printf("{\"kind\":\"pressure-provider-adapter-probe\",\"status\":\"ok\","
	    "\"data\":{\"label\":\"%s\",\"sample_count\":%" PRIu64 ","
	    "\"interval_ms\":%u,\"duration_ms\":%u},\"meta\":{"
	    "\"component\":\"c\",\"binary\":\"twq-pressure-provider-adapter-probe\"}}\n",
	    label, sample_count, interval_ms, duration_ms);
	fflush(stdout);
}

static void
emit_adapter_snapshot(const char *label, uint32_t interval_ms,
    uint32_t duration_ms, const struct twq_pressure_provider_view_v1 *view)
{
	printf("{\"kind\":\"pressure-provider-adapter-snapshot\",\"status\":\"ok\","
	    ...
	    duration_ms, view->struct_size, view->version, view->generation,
	    view->monotonic_time_ns, view->request_events_total,
	    view->worker_entries_total, view->worker_returns_total,
	    view->requested_workers_total, view->admitted_workers_total,
	    view->blocked_events_total, view->unblocked_events_total,
	    view->blocked_workers_total, view->unblocked_workers_total,
	    view->total_workers_current, view->idle_workers_current,
	    view->nonidle_workers_current, view->active_workers_current,
	    view->should_narrow_true_total, view->request_backlog_total,
...
main(int argc, char **argv)
{
	...
	for (;;) {
		...
		emit_adapter_snapshot(label, interval_ms, duration_ms, &view);
