...
usage() {
  cat <<'EOF'
Usage: run-workqueue-wake-bench.sh [--help]
...
Environment:
  ...
  TWQ_WORKQUEUE_WAKE_MODE            Benchmark mode: wake-default, wake-overcommit
  TWQ_WORKQUEUE_WAKE_SAMPLES         Sample count
  TWQ_WORKQUEUE_WAKE_WARMUP          Warmup count
  ...
  TWQ_WORKQUEUE_WAKE_BENCH_PLAN      Optional host plan file with one benchmark arg line per run
EOF
}
...
bench_mode=${TWQ_WORKQUEUE_WAKE_MODE:-wake-default}
bench_samples=${TWQ_WORKQUEUE_WAKE_SAMPLES:-256}
bench_warmup=${TWQ_WORKQUEUE_WAKE_WARMUP:-32}
bench_settle_ms=${TWQ_WORKQUEUE_WAKE_SETTLE_MS:-50}
bench_prime_timeout_ms=${TWQ_WORKQUEUE_WAKE_PRIME_TIMEOUT_MS:-3000}
bench_callback_timeout_ms=${TWQ_WORKQUEUE_WAKE_CALLBACK_TIMEOUT_MS:-3000}
bench_quiescent_timeout_ms=${TWQ_WORKQUEUE_WAKE_QUIESCENT_TIMEOUT_MS:-3000}
bench_plan=${TWQ_WORKQUEUE_WAKE_BENCH_PLAN:-}
...
if [ -n "$bench_plan" ]; then
  ...
  serial_log=${TWQ_SERIAL_LOG:-${artifacts_root}/benchmarks/workqueue-wake-${bench_mode}-${timestamp}.serial.log}
  benchmark_json=${TWQ_BENCHMARK_JSON:-${artifacts_root}/benchmarks/workqueue-wake-${bench_mode}-${timestamp}.json}
  benchmark_label=${TWQ_BENCHMARK_LABEL:-workqueue-wake-${bench_mode}}
fi
...
if [ -n "$bench_plan" ] && [ ! -r "$bench_plan" ]; then
  echo "TWQ_WORKQUEUE_WAKE_BENCH_PLAN is not readable: $bench_plan" >&2
  exit 66
fi
...
if [ -n "$bench_plan" ]; then
  ...
  bench_args="--mode ${bench_mode} --samples ${bench_samples} --warmup ${bench_warmup} --settle-ms ${bench_settle_ms} --prime-timeout-ms ${bench_prime_timeout_ms} --callback-timeout-ms ${bench_callback_timeout_ms} --quiescent-timeout-ms ${bench_quiescent_timeout_ms}"
fi
...
env \
  ...
  TWQ_WORKQUEUE_WAKE_BENCH_ARGS="$bench_args" \
  TWQ_WORKQUEUE_WAKE_BENCH_PLAN="$bench_plan" \
...
echo "==> Running guest workqueue wake benchmark"
run_rc=0
probe_completed=0
env \
  TWQ_VM_IMAGE="$vm_image" \
...
echo "==> Extracting structured workqueue wake benchmark"
extract_args=
if [ -n "$bench_plan" ]; then
  extract_args="--all"
fi
python3 "${repo_root}/scripts/benchmarks/extract-workqueue-wake-bench.py" \
