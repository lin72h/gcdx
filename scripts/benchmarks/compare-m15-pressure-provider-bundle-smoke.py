#!/usr/bin/env python3
"""Compare pressure-provider bundle smoke artifacts against the baseline."""
...
REQUIRED_TRUE_FIELDS = ("generation_contiguous", "monotonic_increasing", "current_quiescent")
...
MIN_RATIO_FIELDS = {
    ...
    # Quiescent samples are a low-count timing tail in sustained mode.  The
    # hard invariants are final quiescence plus quiescent rise/fall transitions.
    "observer_quiescent_samples": 0.25,
...
def compare_capture(label: str, baseline_capture: dict | None, candidate_capture: dict | None):
    ...
    for field in MAX_FIELDS:
        ...
        if lhs is None:
            ...
            status = "fail"
            failure = f"{label}: {field} {rhs} exceeds baseline {lhs}"
            failures.append(failure)
        result["checks"].append(
            {
                "kind": "maximum",
...
def main() -> int:
    ...
    for label in labels:
        ...
        for check in results[label]["checks"]:
            ...
            elif check["kind"] == "maximum":
                print(
                    f"  maximum:{check['field']} baseline={check['baseline_value']} "
                    f"candidate={check['candidate_value']} status={check['status']}"
                )
